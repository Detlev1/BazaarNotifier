...
export default {
  update: async (resources, access_token, log = false) => {
    if(access_token !== ""){
      axios.defaults.headers.common['Authorization'] = `Bearer ${access_token}`
    }
    if(log){
      console.log("Starting Crafting checks")
    }
    ...
    outer: for (let name of Object.keys(resources.bazaarConversions)) {
      ...
      if (name === "ENCHANTED_CARROT_ON_A_STICK" || name === "BAZAAR_COOKIE" || name.startsWith("ENCHANTMENT") || Object.keys(resources.enchantCraftingList.other).includes(name) || resources.invalidBazaarCraftingRecipe.includes(name)) continue
      if(access_token === ""){
        await delay(50);
      }
      ...
      if (response.status === 200) {
        ...
        if ("recipe" in itemData) {
          ...
          if(!Object.keys(resources.invalidBazaarCraftingRecipe).includes(name)){
